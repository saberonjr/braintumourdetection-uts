...
def preprocess_and_upload(dataset_id, register_name, cfg):
    coco_path = 'C:/Users/Leon-PC/Downloads/Brain Tumor Detection Dataset/Dataset/test/_annotations.coco.json'
    coco = COCO(coco_path)
    processed_images_dir = os.path.join('C:/Users/Leon-PC/Downloads/Preprocessed data/Preprocessed_test', register_name)
    os.makedirs(processed_images_dir, exist_ok=True)

    for img_info in coco.dataset['images']:
        image_path = os.path.join('C:/Users/Leon-PC/Downloads/Brain Tumor Detection Dataset/Dataset/test', img_info['file_name'])
    ...
    task = Task.init(project_name="Brain Tumor Detection Project", task_name="Upload PreProcessed test Data", task_type=Task.TaskTypes.data_processing)
    processed_dataset = Dataset.create(dataset_name="PreProcessed Test", dataset_project="Brain Tumor Detection Project", parent_datasets= [dataset_id])
...
def main():
    cfg = setup_cfg()
    preprocess_and_upload('6ec815a3e79b4f7688e530642fd34b38', 'Preprocessed test', cfg)
